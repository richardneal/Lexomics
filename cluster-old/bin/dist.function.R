...
####test calls
#print(fname <- runDist("FOTRnoPoetry.tsv", 2, 2))
...
runDist <- function(input.file, n.clades, n.dist.values = 1, metric = "euclidean",
                  p = 2, linkage = "ave", is.tab.sep=TRUE, output.file = "dist-output", d.metric = 1)
  {
    ...
    #if tmp.dir doesn't exist, create it (it shouldn't exist, but just to be safe)
    ...
    #split file name at "." to get extension out
    f.unlist <- unlist(strsplit(input.file, "\\."))
    #remove extension
    f.name <- f.unlist[(length(f.unlist) - 1)]
    #split f.name at "/" to remove directory from file name
    f.unlist <- unlist(strsplit(f.name, "\\/"))
    #last item in the unlist of f.name is the file name
...
## Sydney's custom score for distances
