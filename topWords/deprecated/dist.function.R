...
#a user-specified distance analysis to determine which words are in outlying clades
#have the most influence in separation on the dendro.  Number of cuts to perform
#and how many levels of distance statistic to look at will be passed in
...
#fname <- runDist("/home/raffle/work/DAZ/merge_transpose_DAN450_AZcb_kt.tsv",2, 2, output.file = "DAZ_450-dist")
...
runDist <- function(input.file, n.clades, n.dist.values = 1, metric = "euclidean",
                  p = 2, linkage = "ave", is.tab.sep=TRUE, output.file = "dist-output", d.metric = 1)
  {
    ...
    tmp.dir <- paste("/tmp/", (1000 * round(runif(1, 0, 1), 3)), sep="")
