...
$outfile	= $_POST['outfile'];// outfile name
...
$pattern='/[^a-zA-Z0-9]/';
$outfile=preg_replace($pattern,"_",$outfile);
// in the case that the user does not provide an output file name
if ($outfile=='') $outfile="dist-output";
...
$rArgs = "$infile $clades $levels $metric $p $linkage $outfile $distance $tsv";
...
if (file_exists($file)) {
	...
	header("Content-Disposition: attachment; filename=$outfile.zip");
